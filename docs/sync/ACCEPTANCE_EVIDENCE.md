...
> Evidence snapshot: 2026-07-13 · WebObsidian implementation `3d27b5a` · plugin source/tag `d7b5d80`/`0.1.5` ·
...
| Native plugin release | [`central-vault-sync` 0.1.5](https://github.com/picassio/central-vault-sync/releases/tag/0.1.5), source/tag `d7b5d80`; release CI [29245150078](https://github.com/picassio/central-vault-sync/actions/runs/29245150078), plus Node 20/22/24 CI [29245148526](https://github.com/picassio/central-vault-sync/actions/runs/29245148526). |
| Real Obsidian Linux | [`evidence/obsidian-linux-1.12.7-plugin-0.1.5-release.png`](evidence/obsidian-linux-1.12.7-plugin-0.1.5-release.png) and its [evidence notes](evidence/README.md): exact public 0.1.5 bytes retain the complete 0.1.4 lifecycle matrix and additionally prove a normal foreground Vault event during an outage automatically retries after server return without manual sync or restart. |
...
| 9 | **PARTIAL** | Native desktop implementation: `central-vault-sync/src/main.ts`, `local-queue.ts`, `obsidian-adapter.ts`, `plugin-store.ts`; plugin tests/conformance pass. Exact 0.1.5 bytes on real Linux Obsidian 1.12.7 cover Markdown and binary attachments, modify, rename, delete, concurrent uploads, server outage, hard restart, offline cold start, automatic startup/runtime retry, and exact convergence with no conflict/queue residue. | Repeat the copied-vault desktop matrix on real Windows and macOS and retain diagnostics/results. Linux is complete, but unavailable platforms are required by M36.8/stable matrix. |
...
| 36 — native plugin | **PARTIAL** | M36.1–M36.9 implementation/release work and the full real-Linux lifecycle/file-operation matrix are complete; public 0.1.5 exists. | M36.2 npm core publication, M36.7 mobile physical lifecycle, M36.8 Windows/macOS/mobile matrix, and M36.10 Community review remain open. |
