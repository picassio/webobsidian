...
> Evidence snapshot: 2026-07-13 · WebObsidian source `3d27b5a` (documentation follow-up `cdec593`) ·
...
| Native plugin release | [`central-vault-sync` 0.1.3](https://github.com/picassio/central-vault-sync/releases/tag/0.1.3), source/tag `6627954`; release CI 29232796996. |
| Real Obsidian Linux | [`evidence/obsidian-linux-1.12.7-plugin-0.1.2-release.png`](evidence/obsidian-linux-1.12.7-plugin-0.1.2-release.png) and its [evidence notes](evidence/README.md): exact public release bytes, pairing, device write, remote apply, cursor/queue convergence. Earlier hard-restart/offline evidence is also recorded there. |
...
| 9 | **PARTIAL** | Native desktop implementation: `central-vault-sync/src/main.ts`, `local-queue.ts`, `obsidian-adapter.ts`, `plugin-store.ts`; plugin tests/conformance pass. Real Linux Obsidian 1.12.7 release bytes pair/push/pull and survive server outage plus hard restart (screenshots above). | Complete the copied-vault desktop matrix for attachment, rename, and delete on real Windows and macOS; retain diagnostics/results. Linux real evidence exists, but unavailable platforms are required by M36.8/stable matrix. |
...
| 36 — native plugin | **PARTIAL** | M36.1–M36.9 implementation/release work and real Linux evidence are complete; public 0.1.3 exists. | M36.2 npm core publication, M36.7 mobile physical lifecycle, M36.8 Windows/macOS/mobile matrix, and M36.10 Community review remain open. |
