...
> Evidence snapshot: 2026-07-14 · WebObsidian implementation `7ab4e4a` · plugin source/tag `26cd75e`/`0.1.14` ·
...
| Native plugin release | [`central-vault-sync` 0.1.14](https://github.com/picassio/central-vault-sync/releases/tag/0.1.14), source/tag `26cd75e`; release CI [29311542151](https://github.com/picassio/central-vault-sync/actions/runs/29311542151), plus Node 20/22/24 CI [29311540042](https://github.com/picassio/central-vault-sync/actions/runs/29311540042) attempt 2 (attempt 1 hit transient npm `ECONNRESET`). It uses popout-compatible `activeDocument`/`activeWindow`; release SHA-256 values are `92c6eaa2…243d` (`main.js`), `0ac542af…bc1` (`manifest.json`), and `4759b965…fb4d` (`styles.css`). It consumes public `@picassio/sync-core@0.1.3`. |
...
| 14 | **BLOCKED** | Plugin 0.1.14 is a normal public non-draft/non-prerelease GitHub release and its Community entry is live with automated review pending; `@picassio/sync-core@0.1.3` and `web-vault-sync@0.1.0` are public and registry-origin verified; Dockerfiles are reproducibly built/smoked for amd64/arm64 from source. | (1) Submit plugin to Obsidian Community Plugins, resolve review, install/update from the in-app directory. (2) Complete unavailable platform/independent beta gates. (3) Create the aligned stable tag and verify GitHub release checksums/SBOM/attestations plus npm provenance. |
...
| 36 — native plugin | **PARTIAL** | M36.1–M36.9 implementation/release work, public `@picassio/sync-core`, popout-compatible plugin 0.1.14, and the full deployed real-Linux 0.1.12 lifecycle/file-operation/editor-safety matrix are complete. Community entry is live and automated review is running. | M36.7 mobile physical lifecycle, M36.8 Windows/macOS/mobile matrix, and M36.10 reviewer approval plus in-app install/update remain open. |
...
|---|:---:|---|
| Settings/runtime/API/UI implementation | **PASS** | PRD 1.9, `docs/MULTI_VAULT.md`, settings v4 in-place migration, detached identity-preserving registry, AsyncLocalStorage request context, per-vault runtime/index/watcher/Git/share/workspace state, token/API-key scoping, selector and deep links. |
...
1. **Obsidian Community review:** the entry for
   ...
   on 0.1.14. Resolve remaining automated/human feedback and verify in-app Community Plugins install/update.
