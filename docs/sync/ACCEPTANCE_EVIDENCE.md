...
> Evidence snapshot: 2026-07-14 · WebObsidian implementation `06cdbf7` · plugin source/tag `121e03b`/`0.1.15` ·
...
|---|:---:|---|
| Settings/runtime/API/UI implementation | **PASS** | PRD 1.12, `docs/MULTI_VAULT.md`, settings v4 in-place migration, detached identity-preserving registry, AsyncLocalStorage request context, per-vault runtime/index/watcher/Git/share/workspace state, token/API-key scoping, selector and deep links. |
...
| Deployment/release | **PASS** | Multi-vault base `7ab4e4a`, pairing-safety `b9e6cae`, vault/device-control `3b2c81b`, and managed-create `06cdbf7` are pushed/deployed; CI 29328809689 passes both jobs (attempt 1 was cancelled after an abnormally stalled unchanged Buildx step). Stopped-service backups, stable v3→v4 identity/sequence and byte-identical default-vault verification, rendered two-vault switching, same-path isolation, forged-header token binding, detach/re-register identity, old-build/v3 rollback, forward-v4 restore, full guest reboot, trusted HTTPS, permanent headless sync and clean 637-entry doctor all pass. A later empty `Desktop Obsidian` production vault was registered independently and rendered/API verification showed selector isolation, healthy sequence 0, zero lag, and vault-bound pairing-code issuance. A subsequently mis-targeted desktop code committed only 30 creates + 10 mkdirs to the default vault before rate limiting; the device was revoked, a stopped-service backup was taken, journal/manifest audit proved no existing-entry modification or collision, and all 40 paths were removed through revisioned delete/rmdir operations. Server and Pi PARA canonical paths were clean at recovery sequence/cursor 845; the isolated desktop vault remained empty. Pi PARA was subsequently moved by explicit owner request to a newly managed isolated `pi-para-claude-3` vault, healthy at sequence/cursor 638 with 632 checked entries and zero doctor issues/pending/error/lag; default has no active device and Desktop Obsidian remains empty. A mount-source mismatch was caught before clients resumed; authoritative storage was untouched and the unused empty volume was removed. | None for Phase 41. Publishing the headless watcher patch remains a separate Phase 37/40 stable-release gate. |

Phase 41 does not change Protocol 1.0 client request shapes. Base `7ab4e4a`, pairing-safety `b9e6cae`, vault/device controls `3b2c81b`, managed-create `06cdbf7`, CI 29328809689, and the deployment/rollback/reboot record make this immutable public evidence.
