...
- **Authoritative server:** Vault trên WebObsidian server là nguồn sự thật. Mọi mutation từ web, Agent API,
  plugin native, daemon headless, watcher filesystem và Git restore/import phải đi qua một `SyncCoordinator`.
  Coordinator/revision journal luôn bật; pairing chỉ mở khi settings v3 có `sync.enabled=true` và
  `sync.bootstrapState=ready`. Gate này không được tắt stale-write protection cho web/agent.
...
- **Device auth:** pairing code random, hash, TTL ngắn, single-use → device token scope `sync`; token hash server-side,
  list/revoke/last-seen; không dùng master password hay broad Agent key làm sync token mặc định.
- **Storage JSON-only:** `data/sync/` gồm vault metadata, revision index, devices, idempotency cache, write-ahead
