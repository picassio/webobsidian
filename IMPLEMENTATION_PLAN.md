...
Cập nhật lần cuối: 2026-07-17 (M36.10 review remediation: plugin 0.1.22 temporarily omits rejected attestations; Community rescan pending)
...
- [x] M41.11 Default vault normalization (PRD 1.13): name default record `Default`, canonical root `/vaults/default`,
...
### Nhật ký tiến độ
- 2026-07-17 (M36.10 non-blocking attestation fallback): Community automation rejected newly issued 0.1.21 v3
  provenance too, so the issue is not limited to v4 or artifact/tag shape. Public scorecard comparison proves that a
  release with no GitHub artifact attestation receives a non-blocking `Other` disclosure and can pass review, whereas
  any newly issued bundle for this plugin is classified as a hard invalid-signature risk despite GitHub verification.
  Immutable 0.1.22 source/tag `8c402c3` therefore temporarily omits attestations for Community review while retaining
  lightweight tags, unique asset digests, source reproducibility, exact release/source byte checks, and CI. Each new
  digest has zero associated attestations by API. CI 29561541163/release CI 29561587726 pass. This does not waive
  stable-release provenance: restore compatible attestations before stable acceptance. Community rescan is pending.
- 2026-07-17 (M36.10 provenance-version boundary): deeper comparison found the concrete compatibility boundary rather
  than an asset/signature defect. Community scorecards verify older releases generated by `actions/attest-build-provenance`
  v2/v3 (examples: Copilot 3.3.3 and Typefully 3.3.1) while rejecting current v4 statements across multiple repositories
  (Central Vault Sync and MCP Connector 0.27.10), even though GitHub verifies all of them. Immutable 0.1.21 source/tag
  `4cf5648` pins exact accepted v3 action commit `977bb373…e068dcc2a`; its nested predicate/attest commits match the passing
  Typefully run. It uses the proven three-asset statement shape, lightweight tag, unique digests, and exact source identity.
  CI 29559756685/release CI 29559795364 pass; downloaded assets byte-match source and strict verification passes.
  Community rescan is pending. Keep older immutable releases; do not delete scan evidence.
...
- 2026-07-14 (Default root normalization + spacing): PRD 1.13/commit `ecfa31c` names fresh/migrated initial
  ...
  manifest prove 674 files/688 entries/4,258,206 bytes unchanged (`a64d7f01…e1d`); old runtime booted against the
  normalized root before forward restore. A stale deployment-local alias override was caught by mount assertions and
  corrected before clients resumed. Rendered UI shows `Default` at `/vaults/default`; Create vault action spacing is
  visually separated from Register existing. Full local tests/typecheck/docs/Compose checks and CI 29334476573 pass;
  all three production vaults are healthy, Pi PARA remains isolated and clean, and its timer is active.
