...
| `central-vault-sync` | 0.1.22 | 1.0 | Obsidian ≥1.11.4 desktop/mobile | Normal public Community-review release from a lightweight commit tag with unique digests and exact source-built assets. Artifact attestations are temporarily omitted because newly issued v3/v4 bundles are rejected by the current Community scorecard; stable provenance remains required before stable release. It retains the 0.1.17 runtime and adds bounded four-upload bootstrap, capability-gated max-100 ordered publication, resumed seven-phase aggregate progress, safe one-request fallback for older servers, and keeps descendants pending when a parent marker changes during preparation. It displays the bound server vault ID, warns that pairing codes—not device names—select an existing server vault, and reports Retry-After for rate limits; 0.1.14 added popout-compatible foreground handling. Core sync behavior retains the exact implementation that passed deployed pair/push/pull, Markdown/binary, immediate rename→modify, delete, outage/offline hard restart/retry, and unsaved-open-editor conflict preservation on real Linux Obsidian 1.12.7. Community acceptance and other OS/mobile matrix pending. |
...
| `@picassio/sync-core` | 0.1.4 | 1.0 | Platform-neutral ESM | Adds capability-gated ordered operation batches, atomic terminal-batch persistence fast path, and safe lifecycle progress hooks while retaining one-request fallback for older servers. |
