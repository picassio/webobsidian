...
> Evidence snapshot: 2026-07-14 · WebObsidian implementation `7ab4e4a` · plugin source/tag `ab77e27`/`0.1.13` ·
...
| Native plugin release | [`central-vault-sync` 0.1.13](https://github.com/picassio/central-vault-sync/releases/tag/0.1.13), source/tag `ab77e27`; release CI [29307703754](https://github.com/picassio/central-vault-sync/actions/runs/29307703754), plus Node 20/22/24 CI [29307701871](https://github.com/picassio/central-vault-sync/actions/runs/29307701871). It fixes Community review metadata/disclosures only: `main.js` and `styles.css` are byte-identical to tested 0.1.12; release manifest SHA-256 is `148dbc38…962a`. It consumes public `@picassio/sync-core@0.1.3`. |
...
| 14 | **BLOCKED** | Plugin 0.1.13 is a public non-draft/non-prerelease GitHub review release and its Community entry is live with automated review pending (the 0.x software remains beta); `@picassio/sync-core@0.1.3` and `web-vault-sync@0.1.0` are public and registry-origin verified; Dockerfiles are reproducibly built/smoked for amd64/arm64 from source. | (1) Submit plugin to Obsidian Community Plugins, resolve review, install/update from the in-app directory. (2) Complete unavailable platform/independent beta gates. (3) Create the aligned stable tag and verify GitHub release checksums/SBOM/attestations plus npm provenance. |
...
| 36 — native plugin | **PARTIAL** | M36.1–M36.9 implementation/release work, public `@picassio/sync-core`, metadata-only plugin 0.1.13, and the full deployed real-Linux 0.1.12 lifecycle/file-operation/editor-safety matrix are complete. Community entry is live and automated review is running. | M36.7 mobile physical lifecycle, M36.8 Windows/macOS/mobile matrix, and M36.10 reviewer approval plus in-app install/update remain open. |
...
1. **Obsidian Community review:** the entry for
   ...
   on 0.1.13. Resolve remaining automated/human feedback and verify in-app Community Plugins install/update.
